...
ts="$(date +%Y%m%d-%H%M%S)"
logfile_ventral="${logdir}/ventral_distract_min${min_num}_max${max_num}_glim${n_glimpses}_task${task}_${ts}.log"
logfile_main="${logdir}/main_distract_min${min_num}_max${max_num}_glim${n_glimpses}_task${task}_${ts}.log"
...
# --- Run ventral.py ---
python3 ventral.py \
  --model_type=cnn --policy=cheat+jitter --logpolar --loss=mse --solarize --challenge=distract012 \
...
python3 main.py \
  ...
  --use_loss=both --opt=Adam --wd=0.00001 --distinctive=0.3 --challenge=distract012 --task_type="${task}" \
  --shape_input=logpolar --min_num="${min_num}" --max_num="${max_num}" \
