...
echo "Running min=${min_num}, max=${max_num}, glimpses=${n_glimpses}, task=${task}, head=${head}, count_mode=${count_mode}"
...
python3 main.py \
  ...
  --act=lrelu --dropout=0.5 --rep=0 --grid=6 --map_shape_count=4 --head="${head}" --count_mode="${count_mode}" \
  | tee "${logfile_main}"
