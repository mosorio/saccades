...
#   --use_loss=map --opt=Adam --wd=0.00001 --distinctive=0.3 --task_type="${task}" \
...
#   --noise_level=0.74 --train_size=100 --test_size=50 --n_epochs=30 --seed=1 --map_mode=fixed_absence \
...
for map_mode in fixed_absence both bce ce; do  
    logdir="./logs"
    mkdir -p "$logdir"
    ts="$(date +%Y%m%d-%H%M%S)"
...
for map_mode in fixed_absence both bce ce; do  
    ...
    mkdir -p "$logdir"
    ts="$(date +%Y%m%d-%H%M%S)"
    logfile_main="${logdir}/main_onehot_min${min_num}_max${max_num}_glim${n_glimpses}_task${task}_head${head}_count_mode${count_mode}_seed${seed}_map_mode${map_mode}_ce_weight${ce_weight}_bce_weight${bce_weight}_pos_weight=True_${ts}.log"
